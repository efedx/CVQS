...
//    JwtValidaitonService jwtValidaitonService;
    ...
    public ResponseEntity<String> registerEmployee(@RequestHeader("Authorization") String authorizationHeader) {
    ...
    @GetMapping("/test") // todo only admins can do that
    public ResponseEntity<Set<Roles>> test() {
        String username = employeeRepository.findByUsername("1").get().getUsername();
        Set<Roles> roles = employeeRepository.findByUsername("1").get().getRoles();
